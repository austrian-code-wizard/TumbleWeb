...
class RestApiTest(unittest.TestCase):
    ...
    def test_get_datapoints_by_datasource_and_run_interval(self):
        response = self.app.post("/add-tumbleweed", json=self.tumbleweed_json)
        response = self.app.post("/add-subSystem/1", json=self.subSystem_json)
        self.dataSource_json["dtype"] = "F"
        response = self.app.post("/add-dataSource/1", json=self.dataSource_json)
        response = self.app.post("/add-tumblebase", json=self.tumblebase_json)
        response = self.app.post("/start-run/1", json=self.run_json)
        self.floatdatapoint_json["receiving_start"] = datetime.now(timezone.utc).isoformat()
        self.floatdatapoint_json["receiving_done"] = datetime.now(timezone.utc).isoformat()
        self.floatdatapoint_json["message_id"] = 1
        response = self.app.post(f"/add-datapoint/1234567890123456/123456789123457/T1", json=self.floatdatapoint_json)
        t_start = datetime.now(timezone.utc).isoformat()
        self.floatdatapoint_json["receiving_start"] = datetime.now(timezone.utc).isoformat()
        self.floatdatapoint_json["receiving_done"] = datetime.now(timezone.utc).isoformat()
        self.floatdatapoint_json["message_id"] = 2
        response = self.app.post(f"/add-datapoint/1234567890123456/123456789123457/T1", json=self.floatdatapoint_json)
        self.floatdatapoint_json["receiving_start"] = datetime.now(timezone.utc).isoformat()
        self.floatdatapoint_json["receiving_done"] = datetime.now(timezone.utc).isoformat()
        self.floatdatapoint_json["message_id"] = 3
        response = self.app.post(f"/add-datapoint/1234567890123456/123456789123457/T1", json=self.floatdatapoint_json)
        t_end = datetime.now(timezone.utc).isoformat()
        self.floatdatapoint_json["receiving_start"] = datetime.now(timezone.utc).isoformat()
        self.floatdatapoint_json["receiving_done"] = datetime.now(timezone.utc).isoformat()
        self.floatdatapoint_json["message_id"] = 4
        response = self.app.post(f"/add-datapoint/1234567890123456/123456789123457/T1", json=self.floatdatapoint_json)
        self.floatdatapoint_json["receiving_start"] = datetime.now(timezone.utc).isoformat()
        self.floatdatapoint_json["receiving_done"] = datetime.now(timezone.utc).isoformat()
        self.floatdatapoint_json["message_id"] = 5
        response = self.app.post(f"/add-datapoint/1234567890123456/123456789123457/T1", json=self.floatdatapoint_json)
        response = self.app.get(f"/get-datapoints-by-dataSource-and-run-interval/1/1/null/{t_start}")
        self.assertEqual(response.status, "200 OK")
        self.assertIsInstance(response.json, list)
        ...
        self.assertEqual(response.json[0]["packets"], self.floatdatapoint_json["packets"])
        self.assertEqual(response.json[0]["data"], self.floatdatapoint_json["data"])
        self.assertEqual(response.json[0]["message_id"], 1)
        response = self.app.get(f"/get-datapoints-by-dataSource-and-run-interval/1/1/{t_start}/{t_end}")
        self.assertEqual(response.status, "200 OK")
        self.assertIsInstance(response.json, list)
        ...
        self.assertEqual(response.json[0]["packets"], self.floatdatapoint_json["packets"])
        self.assertEqual(response.json[0]["data"], self.floatdatapoint_json["data"])
        self.assertEqual(response.json[0]["message_id"], 2)
        self.assertEqual(response.json[1]["packets_received"], self.floatdatapoint_json["packets_received"])
        self.assertEqual(response.json[1]["packets"], self.floatdatapoint_json["packets"])
        self.assertEqual(response.json[1]["data"], self.floatdatapoint_json["data"])
        self.assertEqual(response.json[1]["message_id"], 3)
        response = self.app.get(f"/get-datapoints-by-dataSource-and-run-interval/1/1/{t_end}/null")
        self.assertEqual(response.status, "200 OK")
        self.assertIsInstance(response.json, list)
        self.assertEqual(len(response.json), 2)
        self.assertEqual(response.json[0]["packets_received"], self.floatdatapoint_json["packets_received"])
        self.assertEqual(response.json[0]["packets"], self.floatdatapoint_json["packets"])
        self.assertEqual(response.json[0]["data"], self.floatdatapoint_json["data"])
        self.assertEqual(response.json[0]["message_id"], 4)
        self.assertEqual(response.json[1]["packets_received"], self.floatdatapoint_json["packets_received"])
        self.assertEqual(response.json[1]["packets"], self.floatdatapoint_json["packets"])
        self.assertEqual(response.json[1]["data"], self.floatdatapoint_json["data"])
        self.assertEqual(response.json[1]["message_id"], 5)
